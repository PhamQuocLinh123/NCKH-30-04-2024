...
class Student:
    def __init__(self, root):
        ...
        my_cursor.execute("Select Class from class")  # truy vấn tất cả Phòng trong bảng class
        data_class = my_cursor.fetchall()
        for i in data_class:  # Với mỗi Phòng trong mảng Phòng : truyền thông tin Phòng vào mảng Phòng
            class_array.append(i)

        img3 = PIL.Image.open(r"ImageFaceDetect\bgnt3.jpg")  # Ảnh nền
        ...
        main_frame = Frame(bg_img, bd=2, bg="#36ADF1")  # main frame
        ...
        # frame thông tin Phòng
        class_student_frame = LabelFrame(Left_frame, bd=2, bg="white", relief=RIDGE, text="Thông tin người dùng ",
                                         font=("times new roman", 11, "bold"))
        class_student_frame.place(x=5, y=35, width=640, height=400)

        # Nhập Id người dùng
        studentID_label = Label(class_student_frame, text="Mã người dùng :", font=("times new roman", 11, "bold"),
                                bg="white")
        ...
        save_btn = Button(btn_frame, text="Lưu", command=self.add_data, font=("times new roman", 11, "bold"), bd=0,
        ...
        update_btn = Button(btn_frame, text="Sửa", command=self.update_data, font=("times new roman", 11, "bold"), bd=0,
                            bg="white", cursor='hand2', activebackground='white',
                            width=115, image=self.photobtn1, fg="white", compound="center")
        update_btn.place(x=180, y=3)

        # nút xóa thông tin sinh viên
        delete_btn = Button(btn_frame, text="Xóa", command=self.delete_data, font=("times new roman", 11, "bold"), bd=0,
                            bg="white", cursor='hand2', activebackground='white',
                            width=115, image=self.photobtn1, fg="white", compound="center")
        ...
        take_photo_btn = Button(btn_frame1, text="Thêm khuôn mặt", command=self.generate_dataset,
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="#36ADF1",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=675, y=10, width=610, height=565)
        ...
        # nút tìm kiếm
        search_btn = Button(search_frame, text="Tìm kiếm", font=("times new roman", 11, "bold"),
                            command=self.search_data, bd=0, bg="white", cursor='hand2', activebackground='white',
                            width=105, image=self.photobtn3, fg="white", compound="center")
        search_btn.grid(row=0, column=3, padx=4)

        # nút xem tất cả
        showAll_btn = Button(search_frame, text="Xem tất cả", font=("times new roman", 11, "bold"),
        ...
        table_frame.place(x=5, y=85, width=600, height=450)
        ...
        "id", "name", "div", "roll", "gender", "dob", "email", "phone", "address", "photo"),
        ...
        self.student_table.heading("div", text="Phòng")
        ...
        self.student_table.bind("<ButtonRelease>",
                                self.get_cursor)  # sự kiện khi click vào bảng các thông tin in ra các txtbox
        ...
        # # search
        # self.var_com_searchclass = StringVar()#Loại tìm kiếm Phòng (theo Phòng, tên Phòng)
    ...
    def add_data(self):  # Thêm dữ liệu
        ...
        if self.var_std_name.get() == "" or self.var_std_id.get() == "" or self.var_div.get() == "":
            messagebox.showerror("Error", "Vui lòng nhập đầy đủ thông tin", parent=self.root)
        elif (self.var_div.get() not in arrayClass):  # Phòng ko tồn tại thì thông báo
    ...
    def fetch_data(self):  # Lấy tất cả thông tin người dùng
        ...
        my_cursor.execute(
            "Select student_id,name,class,roll,gender,dob,email,phone,address,photosample from student")
    ...
    def get_cursor(self, event=""):  # Sự kiện khi click vào bảng thì hiện chi tiết thông tin ra các txtbox
        ...
        self.dob_entry.set_date(data[5]),
    ...
    def search_data(self):  # Hàm tìm kiếm
        ...
        if self.var_com_search.get() == "" or self.var_search.get() == "":
            ...
            try:
                ...
                elif (self.var_com_search.get() == "Phòng biên chế"):
    ...
    def add_Classdata(self):  # Thêm Phòng
        ...
        # =========check class=================
        my_cursor.execute("select Class from `class` ")  # Danh sách Phòng
        ckClass = my_cursor.fetchall()
        arrayClass = []
        for chs in ckClass:
            # print(chs[0])
        ...
        elif (self.var_class.get() in arrayClass):  # Kiểm tra xem Phòng nhập vào đã tồn tại chưa
        ...
        if self.var_com_searchclass.get() == "" or self.var_searchclass.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password,
                                               database=self.db.database, port=self.db.port)
                my_cursor = conn.cursor()  # "ID Điểm Danh", "Ngày", "ID người dùng"
                if (self.var_com_searchclass.get() == "Phòng"):
                    self.var_com_searchclass.set("Class")
                elif (self.var_com_searchclass.get() == "Tên Phòng"):
