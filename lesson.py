...
class Lesson:
    def __init__(self,root):
        ...
        img3 = PIL.Image.open(r"ImageFaceDetect\bgnt3.jpg")
        ...
        #====title=========
        self.txt = "Quản lý thông thời gian"
        self.count = 0
        self.text = ''
        ...
        main_frame = Frame(bg_img, bd=2, bg="#36ADF1")
        ...
        Left_frame.place(x=10, y=5, width=390, height=565)

        label_Update_att = Label(Left_frame, bg="#F0FFF0", fg="#483D8B", text="Thông tin thời gian",
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=410, y=5, width=865, height=565)
    ...
    def add_data(self):
        ...
        # Set default class as "CNTT" if not provided
        if self.var_class.get() == "":
            self.var_class.set("CNTT")
        ...
        if self.var_id.get() == "" or self.dob_entry.get_date().strftime(
            ...
            try:
                ...
                self.reset_data()
                conn.close()
                messagebox.showinfo("Thành công", "Thêm thành công", parent=self.root)
        ...
        if self.var_id.get()=="" or self.var_timestart.get()=="" or self.var_timeend.get()=="" \
            ...
            try:
                ...
                if Update>0:
                    ...
                    if not Update:
                        return
                messagebox.showinfo("Thành công","Cập nhật thành công",parent=self.root)
                conn.commit()
                self.fetch_data()
                self.reset_data()
                conn.close()
        ...
        if self.var_com_search.get() == "" or self.var_search.get() == "":
            ...
            try:
                ...
                if self.var_com_search.get() == "":
                    messagebox.showerror("Lỗi", "Vui lòng chọn một lựa chọn để tìm kiếm", parent=self.root)
                else:
                    # Xử lý việc chọn các lựa chọn khác nhau
                    ...
                    my_cursor.execute("select * from lesson where " + str(column_name) + " Like '%" +
