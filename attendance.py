...
class Attendance:
    ...
    def __init__(self,root):
        ...
        #ảnh nền resize
        img3 = PIL.Image.open(r"ImageFaceDetect\bgnt3.jpg")#mở ảnh trong thư mục
        ...
        # ====title=========
        self.txt = "Quản Lý Thông Tin Danh Tính"
        self.count = 0
        # self.text = ''
        self.color = ["#4f4e4d", "#f29844", "red2"]
        self.heading = Label(self.root, text=self.txt, font=("times new roman", 22, "bold"), bg="white", fg="red",
                             bd=5, relief=FLAT)
        self.heading.place(x=350, y=22, width=600)

        #frame chính và vị trí
        main_frame = Frame(bg_img, bd=2, bg="#36ADF1")# nằm trong label bg_img, viền xung quanh 2px, bg: mầu nền trắng
        main_frame.place(x=24, y=90, width=1293, height=586)

        # frame bên trái chứa các label, entry textbox
        Left_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE,
                                font=("times new roman", 12, "bold"))
        Left_frame.place(x=10, y=5, width=390, height=570)
        ...
        classLabel = Label(left_inside_frame, text="Ngành:", font=("times new roman", 12, "bold"),
                                    bg="white")
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=410, y=5, width=870, height=570)
        ...
        self.AttendanceReportTable=ttk.Treeview(table_frame,column=("id","idsv","name","class","time_in","date","lesson","attendance"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.AttendanceReportTable.heading("idsv", text="ID")
        self.AttendanceReportTable.heading("name", text="Tên")
        self.AttendanceReportTable.heading("class", text="Ngành")
    ...
    def fetch_data(self):
        mydata.clear()  # Xóa dữ liệu từ danh sách hiện có
        ...
        my_cursor.execute(
            "SELECT IdAuttendance, Student_id, Name, Class, Time_in, Date FROM attendance")  # Chọn các cột cụ thể
        data = my_cursor.fetchall()

        if len(data) != 0:
            self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())  # Xóa dữ liệu bảng hiện có
            for i in data:
                self.AttendanceReportTable.insert("", END, values=i)  # Chèn dữ liệu đã lấy vào bảng
                mydata.append(i)

        conn.commit()
        my_cursor.close()  # Đóng con trỏ
        conn.close()
    ...
    def exportCsv(self):
        try:
            ...
            df = pd.DataFrame(mydata,
                              columns=['Mã ', 'ID', 'Tên', 'Ngành', 'Giờ vào', 'Ngày'])

            # Đường dẫn và tên file Excel
            excel_file_path = f'D:/Nghiên cứu khoa học-2024/face_recognizer\Thong_Ke\danhsachdanhtinh{ts}_{self.today}.xlsx'
            ...
            # Thông báo
            messagebox.showinfo("Xuất Dữ Liệu", f"Dữ liệu đã được xuất ra tệp Excel: {excel_file_path}")

        except Exception as es:
            messagebox.showerror("Lỗi", f"Xảy ra lỗi: {str(es)}", parent=self.root)
    ...
    def openImage(self):#hàm mở ảnh đã điểm danh
        if self.var_atten_id == "":
            ...
            img = cv2.imread("DiemDanhImage\ " + str(self.var_atten_id.get())+ ".jpg")
            img = cv2.resize(img, (300, 300))
            img1 = cv2.imread("DiemDanhImage\ " + str(self.var_atten_id.get()) + ".jpg")
            img1 = cv2.resize(img1, (300, 300))
    ...
    def search_data(self):
        search_text = self.var_search.get()  # Get the search text from the entry
        ...
        if search_text == "":
            ...
            try:
                ...
                mydata.clear()
                # Execute the query to search for data
                query = f"SELECT IdAuttendance, Student_id, Name, Class, Time_in, Date FROM attendance WHERE IdAuttendance LIKE '%{search_text}%' OR Student_id LIKE '%{search_text}%' OR Name LIKE '%{search_text}%' OR Class LIKE '%{search_text}%' OR Time_in LIKE '%{search_text}%' OR Date LIKE '%{search_text}%'"
                my_cursor.execute(query)

                data = my_cursor.fetchall()
                if len(data) != 0:
                    # If data is found, clear the table and insert the new data
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    for i in data:
                        # Append only the specified columns to mydata
                        mydata.append((i[0], i[1], i[2], i[3], i[4], i[5]))
                        # Insert only the specified columns into the table
                        self.AttendanceReportTable.insert("", END, values=(i[0], i[1], i[2], i[3], i[4], i[5]))
                    # Show a message with the number of records found
                    messagebox.showinfo("Information", f"Tìm được {len(data)} dữ liệu",
                                        parent=self.root)
                    conn.commit()
                else:
                    # If no data found, clear the table and show a message
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    messagebox.showinfo("Information", "Không tìm thấy dữ liệu phù hợp ", parent=self.root)
                conn.close()
            except Exception as es:
